"""Plot family-sponsored visa wait times per country from ``data/raw/`` into
...
for country in countries:
    df = pd.read_csv(f"data/raw/{country.lower()}_family_visa_backlog_timecourse.csv")
    ...
    fig.suptitle(f"Tiempos de espera de visa familiar — {nombre}", fontsize=14, fontweight="bold", color=UACJ_NEGRO)
    ...
    for i, (level, label) in enumerate(zip(f_levels, f_labels, strict=False)):
        ...
        if not data_a.empty:
            ax.plot(
                data_a["visa_bulletin_date"],
                data_a["visa_wait_time"],
        ...
        if not data_b.empty:
            ax.plot(
                data_b["visa_bulletin_date"],
                data_b["visa_wait_time"],
