...
--
-- H1 (plan auditoría 3 repos, 12-jul-2026): this file is the BASELINE of the
-- versioned migration chain — it is applied as migration 001 and must stay
-- byte-identical to pipeline/migrations/001_baseline_star_schema.sql (pinned by
-- tests/test_database_migrations.py). Structural changes AFTER the baseline live
-- in pipeline/migrations/NNN_*.sql (002 adds run/row provenance + deterministic
-- timestamps); the applied chain is recorded in schema_version with per-file
-- sha256 checksums, so silently editing an applied migration aborts the build.
