...
| T | Dependencia Python comprometida o con CVE | **11 locks por perfil × plataforma** (`locks/`; Linux+deep+dvc-tool con `--require-hashes`/`--generate-hashes`, constraints en el perfil model); `pip-audit` sobre los 11 locks reconciliado por `tools/audit_python_supply_chain.py` contra la fuente única `security/python_advisories.json` (**biyección exacta por perfil/lock**; runtime/dev vetan; model/deep/dvc-tool con allowlist explícita; cierra el hueco de versión local torch `+cpu`/`+cu126`); SBOM CycloneDX por corrida; toolchain de instalación PINEADO (`pip`/`setuptools`/`wheel`/`uv`) + promoción del lockset con rollback transaccional + detección de matriz parcial (contrato estático `tools/lock_contracts.py`, manifiesto content-addressed); **CI instala de verdad los locks deep CPU/macOS** (job `deep-lock-install`); pin nuevo ⇒ regenerar locks en el MISMO commit | MEDIO-BAJO: 2 avisos aceptados (1 producto: pytorch-lightning en model/deep; 1 herramienta: diskcache en dvc-tool — ambos OFFLINE sin tráfico, triage en `SECURITY_TRIAGE.md`; P0R.4 subió torch 2.13.0 y cerró CVE-2025-3000; P0R.4R3 subió setuptools 83.0.0 y cerró PYSEC-2026-3447 vía fix; P0R.5 gobernó dvc como perfil `dvc-tool` aislado con `dvc_cache_guard` mitigando PYSEC-2026-2447 —diskcache sin fix, RCE local con escritura previa); hash-checking no cubre el perfil model (constraints no admiten hashes); CUDA en resolución+hash+audit (ejecución A10G en P0R.5) |
