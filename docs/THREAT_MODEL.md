...
| T | Dependencia Python comprometida o con CVE | **9 locks por perfil × plataforma** (`locks/`; Linux+deep con `--require-hashes`/`--generate-hashes`, constraints en el perfil model); `pip-audit` sobre los 9 locks reconciliado por `tools/audit_python_supply_chain.py` contra la fuente única `security/python_advisories.json` (**biyección exacta por perfil/lock**; runtime/dev vetan; model+deep con allowlist explícita; cierra el hueco de versión local torch `+cpu`/`+cu126`); SBOM CycloneDX por corrida; toolchain de instalación PINEADO (`pip`/`setuptools`/`wheel`/`uv`) + promoción del lockset con rollback transaccional + detección de matriz parcial (contrato estático `tools/lock_contracts.py`, manifiesto content-addressed); **CI instala de verdad los locks deep CPU/macOS** (job `deep-lock-install`); pin nuevo ⇒ regenerar locks en el MISMO commit | MEDIO-BAJO: 1 avisos aceptados (pytorch-lightning en perfiles model y deep, superficie OFFLINE sin tráfico — triage en `SECURITY_TRIAGE.md`; P0R.4 subió torch 2.12.1 y cerró CVE-2025-3000, de 9 a 1; P0R.4R3 subió setuptools 83.0.0 y cerró PYSEC-2026-3447 vía fix, no accept — el gate post-merge lo detectó); hash-checking no cubre el perfil model (documentado: constraints no admiten hashes); CUDA queda en resolución+hash+audit (ejecución A10G en P0R.5) |
