...
| P5 | **Registro sombra** (retador naïve-1) | mismo universo servido | h=1..12 | las de P4 (scoring unificado A3: `forecast_scorecard_shadow.csv` + `prospective_head_to_head.json`) | añadas congeladas por el cron desde jul-2026 | Insumo del gate de promoción (A4) vía head-to-head por pares del mismo modo | Promoción (0 pares puntuados aún; la política y la muestra mínima son A4) |
...
| `experiments/score_forecasts.py` (`make score-forecasts`) | Califica AMBOS ledgers vs los cortes reales (`dataset.actuals_F()`) con la misma maquinaria (A3). Campeón → `forecast_scorecard.csv` (+`evaluation_mode`/`model_version` por fila) + `_meta.json` (agregados global / por horizonte / por tabla **anclados al modo backfill**; `by_mode` reporta cada modo por separado y JAMÁS se agregan juntos). Sombra → `forecast_scorecard_shadow.csv` + `_shadow_meta.json`. Comparación por pares del mismo universo y modo → `prospective_head_to_head.json` (insumo del gate A4). Tracking MLflow es local-dev; el registro durable es el scorecard en git. `--demo` corre un self-check sintético. |
