...
| 9 | **EDA imputa con Kalman, nunca rampa sin tope** | `vp_model/features.py::_clean` → `missingness.kalman_impute` (AB1) | Tendencia fabricada contaminando STL/Hurst/changepoints/entropía |
...
| 11 | **Outliers = señal: se cuentan, jamás se recortan** | `features.py::count_outliers` (z-STL) · `n_point_anomalies` (Hampel) · `mega_audit::d9_jumps` (AC2) | Winsorizar borraría las retrogresiones que el modelo debe tolerar |
...
| **Caracterización EDA** (`features._clean`) | ≤3 lineal + huecos largos por **Kalman** (espacio de estados), sin extrapolar bordes | STL/espectro exigen serie completa; Kalman no fabrica tendencia lineal (AB1) |
