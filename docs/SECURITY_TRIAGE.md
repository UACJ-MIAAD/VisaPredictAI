...
audita los **11 locks versionados** con `pip-audit --disable-pip` (sin ocultar nada primero) y
...
| dvc-tool (herramienta CLI, aislada del producto) | `locks/dvc-tool-macos-arm64.txt` | `locks/dvc-tool-linux-x86_64.txt` | **Sí** los 2 (`--generate-hashes`) |
...
- **`tools/make_locks.sh` (`make lock`) regenera los 11 locks en UNA tanda** (3 base macOS +
  3 espejos Linux + 3 deep + 2 dvc-tool) y los promueve con `tools/promote_lockset.py` mediante **rollback
...
- **setuptools 81.0.0 → 83.0.0** (`PYSEC-2026-3447` / `CVE-2026-59890` / `GHSA-h35f-9h28-mq5c`,
  ...
  **Descubierto por el gate post-merge** (CI de `push` sobre `d91064a`): pip-audit consulta OSV en
  ...
  chronos 2.3.1 / transformers 5.13.1), lo que habilita **`setuptools 83` como toolchain UNIFORME**;
  aparece en los **7 locks** donde forma parte del cierre (`runtime-linux`/`dev-linux` no lo incluyen:
  uv no lo arrastra como dep de ejecución — 0 exposición a 81, solo no está pinneado ahí).
...
| diskcache 5.6.3 (perfil **dvc-tool**) | PYSEC-2026-2447 (alias CVE-2025-69872 / GHSA-w8v5-vhqr-4h9v; **sin fix**) | MODERATE (CVSS 5.2): DiskCache usa `pickle`; RCE si un atacante ESCRIBE en el dir de caché y la víctima lo LEE después. Transitiva de `dvc-data` usada SOLO por el CLI DVC gobernado (sin imports desde producto). Exposición local: `tools/dvc_cache_guard.py` exige caché solo-usuario dentro del repo, sin symlink/override, `mode & 0o022 == 0`, umask 077 | **Accept** (allowlist, ACOTADO a los 2 locks dvc-tool); vigilar fix upstream | Javier | 2026-08-12 |
