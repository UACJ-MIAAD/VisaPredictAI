...
def _sellar(tmp_path: Path, *, campaign_id: str = "r2_sintetica", sha: str = SHA, sufijo: str = "") -> Path:
    ...
    (base / "reports" / "campaign").mkdir(parents=True)
...
def test_a_properly_accredited_validation_still_authorises_publication(tmp_path: Path) -> None:
    """La mitad que da sentido a los REDs: un recibo real y acreditado SÍ abre la publicación."""
    ruta = _sellar(tmp_path)
...
def test_arbitrary_markdown_with_the_right_hash_does_not_publish(tmp_path: Path) -> None:
...
def test_a_valid_receipt_from_another_campaign_does_not_publish(tmp_path: Path) -> None:
    """★ JSON impecable, esquema correcto… de OTRA corrida. Reutilizar un recibo es forjar."""
    otra = _sellar(tmp_path, campaign_id="otra_campana", sha=OTRO_SHA, sufijo="_otra")
...
def test_a_forged_validated_state_with_an_automated_reviewer_does_not_publish(tmp_path: Path) -> None:
...
def test_a_receipt_predating_the_computation_does_not_publish(tmp_path: Path) -> None:
...
def test_the_receipt_cannot_be_swapped_after_validation(tmp_path: Path) -> None:
...
def test_the_guard_subcommand_refuses_the_same_forgeries(tmp_path: Path) -> None:
    """La puerta no es sólo una función: `txn guard` es lo que invoca el publicador real."""
    ruta = _sellar(tmp_path)
    suelto = tmp_path / "recibo.md"
    suelto.write_text("revisión humana\n", encoding="utf-8")
    _forjar_validated(ruta, suelto)
    assert txn.main(["--path", str(ruta), "guard"]) == txn.EXIT_BLOCKED

    recibo = _acta(ruta, tmp_path / "acta_ok.json")
    _forjar_validated(ruta, recibo)
    assert txn.main(["--path", str(ruta), "guard"]) == txn.EXIT_OK
